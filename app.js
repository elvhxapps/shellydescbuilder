    ...
    btnWrap.innerHTML = state.extraButtons.map((b, i) => `
      <div class="item" data-id="${b.id}">
        ...
        <div class="grid">
          ...
          <div><label>Icon URL</label><textarea rows="2" class="mono" data-type="btn" data-k="iconUrl">${safeText(b.iconUrl)}</textarea></div>
          <div>
            <label>Style</label>
...
document.addEventListener('click', (e) => {
  ...
  if(btn.dataset.act === 'del') { state.extraButtons = state.extraButtons.filter(x => x.id !== id); syncAll(); }
  if(btn.dataset.act === 'del-vid') { state.extraVideos = state.extraVideos.filter(x => x.id !== id); syncAll(); }
  
  if(btn.dataset.act === 'up' || btn.dataset.act === 'down') {
    const i = state.extraButtons.findIndex(x => x.id === id);
    if(i > 0 && btn.dataset.act === 'up') { [state.extraButtons[i-1], state.extraButtons[i]] = [state.extraButtons[i], state.extraButtons[i-1]]; syncAll(); }
    if(i < state.extraButtons.length-1 && btn.dataset.act === 'down') { [state.extraButtons[i+1], state.extraButtons[i]] = [state.extraButtons[i], state.extraButtons[i+1]]; syncAll(); }
  }
  if(btn.dataset.act === 'up-vid' || btn.dataset.act === 'down-vid') {
    const i = state.extraVideos.findIndex(x => x.id === id);
    if(i > 0 && btn.dataset.act === 'up-vid') { [state.extraVideos[i-1], state.extraVideos[i]] = [state.extraVideos[i], state.extraVideos[i-1]]; syncAll(); }
    if(i < state.extraVideos.length-1 && btn.dataset.act === 'down-vid') { [state.extraVideos[i+1], state.extraVideos[i]] = [state.extraVideos[i], state.extraVideos[i+1]]; syncAll(); }
  }
});
...
addEvt('formArea', 'input', (e) => {
  if(['INPUT','TEXTAREA','SELECT'].includes(e.target.tagName) && !e.target.dataset.type) syncAll(false);
});

addEvt('btnAddButton', 'click', () => { state.extraButtons.push({ id: uid(), label_gr: 'Νέο κουμπί', label_en: 'New Button', url: '', iconUrl: '', style: 'ghost' }); syncAll(); });
addEvt('btnAddVideo', 'click', () => { state.extraVideos.push({ id: uid(), url: '' }); syncAll(); });
