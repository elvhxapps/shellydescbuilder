...
function buildHtml() {
  ...
  // Diagrams Block
  const diagramsUrl = (state.diagrams?.url || '').trim();
  ...
  const diagramsBlock = diagramsUrl ? `
  <div style="margin-top:14px; background:linear-gradient(90deg, rgba(178,34,34,0.10), rgba(178,34,34,0.03)); border:1px solid rgba(178,34,34,0.18); border-radius:14px; padding:14px 16px;">
    <a href="${safeText(diagramsUrl)}" target="_blank" style="display:flex; align-items:center; gap:12px; text-decoration:none;">
      <span style="width:40px; height:40px; border-radius:12px; display:grid; place-items:center; background:#fff; border:1px solid rgba(178,34,34,0.18); box-shadow:0 6px 16px rgba(0,0,0,0.06);">
        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#B22222" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
...
function readForm() {
  ...
  state.diagrams.url = $('diagramsUrl').value.trim();
  if(state.lang === 'gr') {
    state.diagrams.label_gr = $('diagramsLabel').value;
    state.diagrams.sub_gr = $('diagramsSub').value;
  } else {
    state.diagrams.label_en = $('diagramsLabel').value;
    state.diagrams.sub_en = $('diagramsSub').value;
  }
}
...
function fillForm() {
  ...
  $('diagramsUrl').value = state.diagrams?.url || '';
  $('diagramsLabel').value = state.lang==='gr' ? state.diagrams.label_gr : state.diagrams.label_en;
  $('diagramsSub').value = state.lang==='gr' ? state.diagrams.sub_gr : state.diagrams.sub_en;
...
// Added Event Listeners for Export and Download
...
$('btnDownload').addEventListener('click', () => {
  ...
  a.download = 'elvhx-product-block.html';
  a.click();
...
$('btnDoImport').addEventListener('click', () => {
  const html = $('importArea').value || '';
  // Εδώ ήταν το λάθος (το κενό // αντί για το σωστό regex που ψάχνει το σχόλιο)
  const m = html.match(//);
