document.addEventListener('DOMContentLoaded', () => {
  function defaultState() {
    return {
      ...
      manual: {
        url: '',
        label_gr: 'Οδηγίες χρήσης (PDF)',
        label_en: 'User manual (PDF)'
      },
      video: {
        url: 'https://www.youtube.com/watch?v=nGj5ZgnwP04',
        label_gr: 'Άνοιγμα video',
        label_en: 'Open video'
      },
      diagrams: {
  ...
  function mergeState(parsed) {
    ...
    return {
      ...fresh,
      ...parsed,
      manual: { ...fresh.manual, ...(parsed.manual || {}) },
      video: { ...fresh.video, ...(parsed.video || {}) },
      diagrams: { ...fresh.diagrams, ...(parsed.diagrams || {}) },
      extraButtons: Array.isArray(parsed.extraButtons) ? parsed.extraButtons : [],
      extraVideos: Array.isArray(parsed.extraVideos) ? parsed.extraVideos : [],
      bullets: Array.isArray(parsed.bullets) ? parsed.bullets : fresh.bullets
    };
  }
      ...
      btnWrap.innerHTML = state.extraButtons.map((b, i) => `
        <div class="item" data-id="${b.id}">
          <div class="top">
            ...
            <div class="row">
              <button class="btn small" type="button" data-act="up-btn">↑</button>
              <button class="btn small" type="button" data-act="down-btn">↓</button>
              <button class="btn small danger" type="button" data-act="del-btn">X</button>
            </div>
          </div>
          <div class="grid">
      ...
      vidWrap.innerHTML = state.extraVideos.map((v, i) => `
        <div class="item" data-id="${v.id}">
          <div class="top">
            ...
            <div class="row">
              <button class="btn small" type="button" data-act="up-vid">↑</button>
              <button class="btn small" type="button" data-act="down-vid">↓</button>
              <button class="btn small danger" type="button" data-act="del-vid">X</button>
            </div>
          </div>
          <div>
  ...
  function createBtn(url, label, icon, style) {
    ...
    return `<a href="${safeText(cleanUrl)}" target="_blank" style="${styleCss}">${iconHtml}${safeText(label)}</a>`;
  }

  function buildHtml() {
    const isGR = state.lang === 'gr';

    const imgHtml = state.topImage
      ? `<img src="${safeText(state.topImage)}" alt="Product Image" style="width:100%;border-radius:14px;margin-bottom:16px;display:block;object-fit:cover;">`
      : '';

    const bulletsHtml = state.bullets.length
      ? `<ul style="margin:0 0 14px 18px;padding:0;font-size:14px;line-height:1.55;color:#222;">${state.bullets.map(b => `<li>${safeText(b)}</li>`).join('')}</ul>`
      : '';
        ...
        mainVideoHtml = `
          <div style="flex:1 1 420px;">
            <div style="position:relative;width:100%;padding-top:56.25%;border-radius:14px;overflow:hidden;background:#000;">
              <iframe src="${safeText(ytEmbed)}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen style="position:absolute;inset:0;width:100%;height:100%;"></iframe>
            </div>
          </div>`;
      }
    }
      ...
      const vids = state.extraVideos.map(v => {
        ...
        return `
          <div style="position:relative;width:100%;padding-top:56.25%;border-radius:12px;overflow:hidden;background:#000;">
            <iframe src="${safeText(embed)}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen style="position:absolute;inset:0;width:100%;height:100%;"></iframe>
          </div>`;
      }).join('');
    ...
    const btnList = [];
    if (state.manual.url) {
      btnList.push(createBtn(
        state.manual.url,
        isGR ? state.manual.label_gr : state.manual.label_en,
        'https://www.elvhx.gr/image/catalog/New%20photos%202022/free-click-icon-2384-thumb.png',
        'primary'
      ));
    }
    if (state.video.url) {
      btnList.push(createBtn(state.video.url, isGR ? state.video.label_gr : state.video.label_en, '', 'ghost'));
    }
    ...
    const buttonsRow = filteredBtns.length
      ? `<div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:10px;">${filteredBtns.join('\n')}</div>`
      : '';
  ...
  function syncAll(rebuildLists = true) {
    try {
      ...
      if ($('preview')) $('preview').innerHTML = html.replace(/<!--\s*ELVHX_TOOL_DATA:[\s\S]*?-->/, '');
  ...
  document.addEventListener('input', (e) => {
    ...
    if (target.dataset.type === 'btn') {
      ...
      if (!itemEl) return;
      const item = state.extraButtons.find(x => x.id === itemEl.dataset.id);
      if (item) {
    ...
    if (target.dataset.type === 'vid') {
      const itemEl = target.closest('.item');
      if (!itemEl) return;
      const item = state.extraVideos.find(x => x.id === itemEl.dataset.id);
      if (item) {
        item[target.dataset.k] = target.value;
        syncAll(false);
      }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (act === 'up-btn' || act === 'down-btn') {
      ...
      if (i < 0) return;
      if (act === 'up-btn' && i > 0) {
        [state.extraButtons[i - 1], state.extraButtons[i]] = [state.extraButtons[i], state.extraButtons[i - 1]];
        syncAll();
      }
      if (act === 'down-btn' && i < state.extraButtons.length - 1) {
        [state.extraButtons[i + 1], state.extraButtons[i]] = [state.extraButtons[i], state.extraButtons[i + 1]];
        syncAll();
      }
      return;
    ...
    if (act === 'up-vid' || act === 'down-vid') {
      ...
      if (i < 0) return;
      if (act === 'up-vid' && i > 0) {
        [state.extraVideos[i - 1], state.extraVideos[i]] = [state.extraVideos[i], state.extraVideos[i - 1]];
        syncAll();
      }
      if (act === 'down-vid' && i < state.extraVideos.length - 1) {
        [state.extraVideos[i + 1], state.extraVideos[i]] = [state.extraVideos[i], state.extraVideos[i + 1]];
        syncAll();
      }
    }
  ...
  if ($('btnDoImport')) {
    $('btnDoImport').addEventListener('click', () => {
      ...
      const m = val.match(/<!--\s*ELVHX_TOOL_DATA:([A-Za-z0-9+/=]+)\s*-->/);
      if (m && m[1]) {
        try {
